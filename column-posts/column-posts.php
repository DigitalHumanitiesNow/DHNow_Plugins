...
class ColumnPosts{
	...
	{				
		...
		foreach ($cats as $cat) {
			...
			if ( $cat->count > 0 ) {
				...
				if ($numposts > 0){
					$cp_str = $this->print_endposts($cp_class, $col_cat, $cp_column, $cp_str, $category_link);
				}
			} // end if categories
		} // end foreach cats
	...
	function print_posts($post, $args){
		...
		function print_endposts($class, &$ncol, $cp_column, $pstr, $category_link){
		if ($class == 'C') {
			$pstr .= '</ul> <span style="font-weight:bold;"><a href="' . esc_url($category_link) . '" title="Read More">Read More</a></span>'; 
			$pstr .= '</div><!-- end of Category Column Box-->';
			$ncol ++;
			if ($ncol == $cp_column) {
